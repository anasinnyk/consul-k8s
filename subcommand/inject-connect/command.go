...
func (c *Command) Run(args []string) int {
	...
	err = c.validatePort("-default-merged-metrics-port", c.flagDefaultMergedMetricsPort)
	if err != nil {
		c.UI.Error(err.Error())
		return 1
	}
	err = c.validatePort("-default-prometheus-scrape-port", c.flagDefaultPrometheusScrapePort)
	if err != nil {
		c.UI.Error(err.Error())
		return 1
	}
	...
	injector := connectinject.Handler{
		...
		ConsulSidecarResources:      lifecycleResources,
...
func (c *Command) validatePort(name, flag string) error {
	port, err := strconv.Atoi(flag)
	if err != nil {
		return errors.New(fmt.Sprintf("%s value of %s is not a valid integer.", name, flag))
	}
	if port <= 1023 || port > 65353 {
		return errors.New(fmt.Sprintf("%s value of %d is not in the port range 1024-65353.", name, port))
	}
	return nil
}
